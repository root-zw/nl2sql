                ...
                <div v-else class="assistant-message">
                  ...
                  <div v-if="msg.status !== 'error' && (hasAnyContent(msg) || msg.status === 'pending' || msg.status === 'running')" class="message-result">
                    ...
                    <div v-if="hasThinkingSteps(msg) || (msg.status === 'running' && getThinkingSteps(msg.message_id).length > 0)" 
                         ...
                         :class="{ 'is-collapsed': !isThinkingExpanded(msg.message_id) && !isThinkingActivelyRunning(msg) }">
                      <div 
                        class="thinking-header" 
                        @click="toggleThinkingExpand(msg.message_id)"
                        :class="{ collapsed: !isThinkingExpanded(msg.message_id) && !isThinkingActivelyRunning(msg) }"
                      >
                        <span class="thinking-icon">{{ getThinkingIcon(msg) }}</span>
                        <span class="thinking-title">
                          {{ getThinkingTitle(msg) }}
                        </span>
...
function hasConfirmedDraftSnapshot(snapshot) {
  const confirmedDraft = snapshot?.state?.confirmed_draft
  if (!confirmedDraft || typeof confirmedDraft !== 'object') {
    return false
  }

  if (confirmedDraft.confirmed !== undefined && confirmedDraft.confirmed !== null) {
    return Boolean(confirmedDraft.confirmed)
  }
  if (confirmedDraft.status) {
    return confirmedDraft.status === 'confirmed'
  }

  return Boolean(
    confirmedDraft.natural_language ||
    confirmedDraft.draft_json ||
    (Array.isArray(confirmedDraft.warnings) && confirmedDraft.warnings.length > 0) ||
    (Array.isArray(confirmedDraft.suggestions) && confirmedDraft.suggestions.length > 0)
  )
}
...
function isMessagesContainerNearBottom(threshold = AUTO_SCROLL_BOTTOM_THRESHOLD_PX) {
...
function scrollToBottom({ force = false } = {}) {
  const shouldStickToBottom = force || isMessagesContainerNearBottom()
  nextTick(() => {
...
function isNarrativePendingForMessage(msg) {
  return Boolean(
    msg?.message_id &&
    currentStreamingMessageId.value === msg.message_id &&
    narrativePending.value
  )
}

function hasExecutionResult(msg) {
  return Boolean(msg?.sql_text || msg?.result_data)
}

function isThinkingActivelyRunning(msg) {
  return msg?.status === 'running' &&
    !hasExecutionResult(msg) &&
    !isNarrativePendingForMessage(msg)
}

function getThinkingIcon(msg) {
  if (isNarrativePendingForMessage(msg) && isThinkingExpanded(msg.message_id)) {
    return '📝'
  }
  if (isThinkingActivelyRunning(msg) && isThinkingExpanded(msg.message_id)) {
    return '⏳'
  }
  return '💭'
}

function getThinkingTitle(msg) {
  if (isNarrativePendingForMessage(msg) && isThinkingExpanded(msg.message_id)) {
    return '正在生成分析...'
  }
  if (isThinkingActivelyRunning(msg) && isThinkingExpanded(msg.message_id)) {
...
function canShowResultDetails(msg) {
  ...
  if (isNarrativePendingForMessage(msg)) {
    return false
  }
