...
- 我已把原来的 `R1 / R2 / V / W` 这类短编号改成可读标识
- 真正给人看的内容是方括号或大括号里的中文标签

```mermaid
flowchart TD
    A[用户输入新问题] --> B[创建或恢复当前 query_id]
    B --> C[生成 safe_summary（安全理解摘要）]
    C --> D{表是否稳定}

    D -- 否 --> E[进入 table_resolution（选表确认）]
    E --> F{用户动作或自由文本}
    F -- choose_table（确认使用该表） --> G[基于已确认表重算]
    F -- change_table（换表） --> E
    F -- manual_select_table（手动选表） --> G
    F -- cancel_query（取消查询） --> Z[结束当前查询]
    F -- 自由文本 --> tableReplyResolution[pending_reply_resolution（待确认回复解析）]
    tableReplyResolution -- resolved_to_action（已解析为当前动作） --> F
    tableReplyResolution -- resolved_to_new_query（已解析为新问题） --> A
    tableReplyResolution -- need_clarification（还需澄清） --> tableClarify[追加一轮极短澄清]
    tableClarify --> E

    D -- 是 --> G[基于已确认表重算]

    G --> H[生成 provisional_draft（暂定查询草稿）或待确认草稿]
    H --> I{是否需要 draft_confirmation（查询草稿确认）}

    I -- 是 --> J[进入 draft_confirmation（查询草稿确认）]
    J --> K{用户动作或自由文本}
    K -- confirm_draft（确认查询草稿） --> L[固定 confirmed_draft（已确认查询草稿）]
    K -- revise_field 或 add_context（继续修订） --> G
    K -- change_table（换表） --> E
    K -- cancel_query（取消查询） --> Z
    K -- 自由文本 --> draftReplyResolution[pending_reply_resolution（待确认回复解析）]
    draftReplyResolution -- resolved_to_action（已解析为当前动作） --> K
    draftReplyResolution -- resolved_to_new_query（已解析为新问题） --> A
    draftReplyResolution -- need_clarification（还需澄清） --> draftClarify[追加一轮极短澄清]
    draftClarify --> J

    I -- 否 --> L[固定 confirmed_draft（已确认查询草稿）]

    L --> M{是否需要 execution_guard（执行保护确认）}
    M -- 是 --> N[进入 execution_guard（执行保护确认）]
    M -- 否 --> P[执行 SQL]

    N --> O{用户动作或自由文本}
    O -- approve_execution（批准执行） --> P
    O -- reject_execution（拒绝执行） --> Z
    O -- revise_field（继续修订） --> G
    O -- change_table（换表） --> E
    O -- cancel_query（取消查询） --> Z
    O -- 自由文本 --> guardReplyResolution[pending_reply_resolution（待确认回复解析）]
    guardReplyResolution -- resolved_to_action（已解析为当前动作） --> O
    guardReplyResolution -- resolved_to_new_query（已解析为新问题） --> A
    guardReplyResolution -- need_clarification（还需澄清） --> guardClarify[追加一轮极短澄清]
    guardClarify --> N

    P --> Q[返回结果与解释]
    Q --> S{用户是否继续输入}
    S -- 否 --> T[查询完成]
    S -- 是 --> U[进入 followup_context_resolution（结果后追问上下文解析）]

    U -- continue_on_result（继续分析上一份结果） --> followupContext[继承 followup 型 analysis_context（分析上下文）]
    U -- compare_with_result（基于上一份结果做对比） --> compareContext[继承 compare 型 analysis_context（分析上下文）]
    U -- resolved_to_new_query（已解析为独立新问题） --> A
    U -- need_clarification（还需澄清） --> resultClarify[追加一轮极短澄清]
    resultClarify --> Q
...
```mermaid
flowchart LR
    A[用户输入] --> B[query_sessions（查询会话状态）]
    B --> C[统一确认读模型]
    D[draft_actions（确认动作记录）] --> B
    D --> C

    C --> E[table_resolution（选表确认）]
    C --> F[provisional_draft（暂定查询草稿）]
    C --> G[confirmed_draft（已确认查询草稿）]
    C --> H[execution_guard（执行保护确认）]

    I[followup_context_resolution（结果后追问上下文解析）] --> J[analysis_context（分析上下文）]
    J --> B

    D --> K[learning_events（学习事件）]
    K --> L[runtime_memories（运行时记忆）]
    K --> M[governance_candidates（治理候选）]
```
