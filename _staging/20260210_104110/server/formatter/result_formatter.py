...
class ResultFormatter:
    ...
    def _identify_derived_metric_type(self, metric_name: str, global_rules: List[Dict] = None, 
        ...
        # 统一处理带单位后缀的列名：如 "每亩单价(万元/亩)" -> "每亩单价"
        metric_name_base = metric_name.split('(')[0].strip()
        ...
        if ir and hasattr(ir, 'calculated_fields') and ir.calculated_fields:
            for calc_field in ir.calculated_fields:
                ...
                if alias and (alias == metric_name_base or metric_name.startswith(alias + '(')):
            ...
            if global_rules:
                for rule in global_rules:
                    if rule.get('rule_type') == 'derived_metric':
                        ...
                        if display_name == metric_name_base:
        ...
        try:
            ...
            if field_id.startswith('derived:'):
                ...
                if formatted_rows:
                    for col_key in formatted_rows[0].keys():
                        clean_key = col_key.split('(')[0].strip()
            ...
            if column_names and original_rows and field_display_name:
                for idx, col_name in enumerate(column_names):
                    # 精确匹配
                    if col_name == field_display_name or col_name == field_id:
                        col_total = Decimal(0)
    ...
    def _get_display_name_with_unit(
        ...
        # 如果显示名称已经包含单位（如"每亩单价(万元/亩)"），直接返回
        if unit and f"({unit})" in display_name:
            return display_name
